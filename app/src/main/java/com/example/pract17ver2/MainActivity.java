package com.example.pract17ver2;
...
public class MainActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    public  void onClick(View view) {
        ...
        if (view.getId() == R.id.btnAdd) {
            ...
            cv.put(DBHelper.COLUMN_Animal, animal);
            cv.put(DBHelper.COLUMN_NAME, name);
            cv.put(DBHelper.COLUMN_Size, size);
            cv.put(DBHelper.COLUMN_Weight, weight);
        ...
        if (view.getId() == R.id.btnLoad) {
            animalList.clear();
            userCursor = db.query(DBHelper.TABLE, null, null, null, null, null, null);
            if (userCursor.moveToFirst()) {

                int AnimalIndex = userCursor.getColumnIndex(DBHelper.COLUMN_Animal);
                int NameIndex = userCursor.getColumnIndex(DBHelper.COLUMN_NAME);
                int SizeIndex = userCursor.getColumnIndex(DBHelper.COLUMN_Size);
                int WeightIndex = userCursor.getColumnIndex(DBHelper.COLUMN_Weight);
                do {
